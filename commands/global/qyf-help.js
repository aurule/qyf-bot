...
module.exports = {
  ...
  data: () =>
    new SlashCommandBuilder()
      ...
      .addStringOption(
        (option) =>
          option.setName("topic").setDescription("The topic you want help with")
  ...
  async execute(interaction) {
    ...
    if (topic_name) {
      ...
      if (! topic) return interaction.reply(`No help is available for the topic "${topic_name}"`)
    ...
    if (! command?.help) return interaction.reply(`No help is available for the command "${command_name}"`)

    // return reply with the command's help text
    return interaction.reply(CommandHelpPresenter.present(command))
  },
  help: ({ command_name }) => stripIndent`
