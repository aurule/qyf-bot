...
describe("complete", () => {
  ...
  describe("with one game", () => {
    ...
    it("gets at most 25 games", async () => {
      jest.spyOn(GamesForGuild, "get").mockResolvedValue(
        Array.from(Array(30).keys(), (x, i) => {
          return { name: `partial game ${i}` }
        })
      )
  ...
  describe("with multiple games", () => {
    beforeEach(async () => {
      await guild.createGame({ name: "Test Game Secondus" })
    })
